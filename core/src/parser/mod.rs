...
impl Parser {
    pub fn new(lexer: Lexer, source: &str) -> Self {
        ...
        p.register_prefix(TokenType::Diverge, Parser::parse_diverge_expression);
        p.register_prefix(TokenType::Converge, Parser::parse_converge_expression);
    ...
    fn parse_diverge_expression(&mut self) -> Option<Expression> {
        ...
        if self.peek_token.token_type == TokenType::Each {
    ...
    fn parse_diverge_each(&mut self, tok: Token) -> Option<Expression> {
        self.next_token(); // move to loop variable
        let var = Identifier {
            token: self.curr_token.clone(),
            value: self.curr_token.literal.clone(),
        };
